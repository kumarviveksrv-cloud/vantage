...
(function(){
  ...
  const copyParts=[...pro.querySelectorAll('.prologue-kicker,.prologue-message,.prologue-sub')];
  // The index page contains later !important typography overrides. Establish
  // the true hidden initial state in JS before the first visible frame so the
  // final subtitle cannot flash at startup on mobile or desktop.
  copyParts.forEach(el=>{
    el.style.setProperty('opacity','0','important');
    el.style.setProperty('visibility','hidden','important');
    el.style.setProperty('filter','blur(10px)','important');
  });
  const photo=$('.prologue-photo');
  if(photo){
    photo.style.opacity='0';
    photo.style.visibility='hidden';
  }
  let active=false, started=false, timers=[];
  ...
  /* ---------- Three.js scene ---------- */
  // The current prologue uses the photographic background as its visual layer.
  // Do not create a WebGL context on phones: it adds startup GPU work and can
  // produce a compositor flash while the mobile background is being revealed.
  if(!isMobile && canvas && window.THREE){
  ...
  // Preload the exact background selected by the CSS media query. The prologue
  // is activated only after the asset has had a chance to decode, preventing a
  // one-frame blank/grey compositor flash on mobile.
  ...
  function startReveal(){
    ...
    revealPhoto();
    later(()=>{pro.classList.add('phase-pressure');revealCopy();},1800);
    later(()=>{skip?.classList.add('show');},2000);
    later(()=>{pro.classList.add('countdown');tick(3);},10000);
    later(()=>{tick(2);},11000);
    later(()=>{tick(1);},12000);
    later(endPrologue,13000);
    if(!isMobile && typeof animate==='function'){
      start=performance.now();
      requestAnimationFrame(animate);
    }
  }

  const imageSrc=isMobile?'prologue-bg-mobile.png':'prologue-bg.png';
  const preload=new Image();
  preload.onload=()=>later(startReveal,80);
  preload.onerror=()=>later(startReveal,80);
  preload.src=imageSrc;
  // Fallback in case an image decode/load event is suppressed by the browser.
  later(()=>{if(!started)startReveal();},700);
})();
