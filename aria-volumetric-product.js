/* aria-volumetric-product.js
   Particle face driven by aria-reference.png.
   Replaces the old Three.js/canvas matrix approach.
   Kills concentric rings via injected CSS.
   Reacts to #slbl class state (idle / listening / thinking / speaking).
*/
(function(){
'use strict';

// ── Kill rings immediately ─────────────────────────────────
...
// ── State colours (match existing aria.html palette) ──────
const COLS={
  idle:     [99,  102, 241],
  listening:[124,  58, 237],
  thinking: [165, 180, 252],
...
function setupCanvas(){
  ...
  if(!canvas){
    ...
    const room=document.querySelector('.aria-room')||document.querySelector('.aria-figure')||document.body;
    // hide old face img
  ...
  resize();
  window.addEventListener('resize',()=>{resize();if(rawParts.length)rebuildPositions();});
}

function resize(){
  const p=canvas.parentElement;
  W=canvas.width=p?(p.offsetWidth||560):560;
  H=canvas.height=p?(p.offsetHeight||650):650;
}

// ── Sample image into normalised particle list ────────────
function sampleImage(img){
  // Sample at 220px on the longer side
  const SMAX=220;
  ...
  for(let y=0;y<sh;y+=STEP){
    for(let x=0;x<sw;x+=STEP){
      ...
      // include face tones, skip pure white bg and pure black
      if(lum<32||lum>248)continue;
      rawParts.push({nx:x/sw,ny:y/sh,lum:lum/255});
    }
  }

  // cap at ~7000 particles via weighted random downsample
  if(rawParts.length>7000){
    const f=7000/rawParts.length;
...
function rebuildPositions(){
  ...
  // detect tight bounding box of sampled points
  ...
  const aW=(W-pad*2)*.94, aH=(H-pad*2)*.94;
  let dW,dH;
...
function animate(){
  ...
  // ambient glow
  const grd=ctx.createRadialGradient(W*.5,H*.45,0,W*.5,H*.45,Math.max(W,H)*.52);
...
function init(){
  ...
  img.onload=function(){resize();sampleImage(img);animate();};
  img.onerror=function(){resize();buildFallback();animate();};
}
